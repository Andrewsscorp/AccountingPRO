...
describe('LedgerQueryService - Query Model', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma1.planCuenta.deleteMany({});
    await prisma1.tipoDocumento.deleteMany({});

    // 2.2 Creamos tipo doc
    const tipoDoc = await prisma1.tipoDocumento.create({
      data: {
        codigo: 'RC',
    ...
    testComprobante1 = await prisma1.comprobante.create({
      data: {
        ...
        numero: 1,
        fecha: new Date(),
        concepto: 'Test Comprobante 1',
        estado: 'ASENTADO'
      }
    });

    // 2.4 Creamos cuenta
    testCuenta1 = await prisma1.planCuenta.create({
      data: {
        codigo: '110505',
